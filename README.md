...
shaded and the measured metrics shown beneath; the sigma threshold and hit
tolerance are adjustable so the precision/recall trade-off is visible directly.
Live mode pulls current NOAA data. All metrics come from `src.evaluation`, so
the dashboard and `RESULTS.md` cannot disagree.

The OMNI2 responses for all five storm windows are committed under
...
it matters, and what this project found, written for a reader who has never
heard of the Kp index. Publish it free via GitHub Pages: **Settings → Pages →
Source: Deploy from a branch → `main` / `/docs`**. It then lives at
`https://chrishoangle.github.io/space-weather-anomaly-detection/`.

No build step, no dependencies, light and dark modes.

### Deploying the dashboard

The app runs on [Streamlit Community Cloud](https://share.streamlit.io) with no
changes:
...
Historical mode works immediately on a cold start because the OMNI2 cache is in
the repository. Live mode calls `services.swpc.noaa.gov` at request time and
degrades to an explicit error message if NOAA is unreachable.
